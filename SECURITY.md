...
- user-confirmed wallpaper deletion accepts only explicitly registered files on local fixed Windows volumes; UNC, mapped/remote, removable/unknown, and observed reparse/provider paths are rejected,
- deletion goes through a project-owned Windows Recycle Bin Shell operation; PureWall never auto-confirms a permanent-delete prompt, requires observed Recycle Bin evidence before metadata cleanup, and treats an explicit permanent-delete choice or missing evidence as unknown with metadata retained,
- Windows Shell deletion is path-bound rather than handle-bound: an active same-user process can still replace a path after final validation and during the Shell operation, so this residual race is mitigated but not eliminated,
